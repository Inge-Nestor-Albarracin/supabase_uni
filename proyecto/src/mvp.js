...
export function mostrarMVP() {
  ...
  // 🔹 Cargar actividades del usuario (CORREGIDO)
  async function cargarActividades() {
    ...
    // ✅ CORREGIDO: usar 'description' en lugar de 'descripcion'
    const { data, error } = await supabase
      .from('actividades')
      .select('id, titulo, description, tipo, imagen')
      .eq('estudiante_id', user.id)
      .order('creado_en', { ascending: false }); // Mejor ordenar por fecha
    ...
    data.forEach(act => {
      ...
      // ✅ CORREGIDO: usar act.description
      div.innerHTML = `
        <hr>
        <h4>${act.titulo}</h4>
        <p>${act.description || ''}</p>
  ...
  // 🔹 Subir nueva actividad (CORREGIDO)
  form.addEventListener('submit', async (e) => {
    ...
    // ✅ CORREGIDO: sintaxis correcta del INSERT
    const { error } = await supabase.from('actividades').insert({
      titulo,
      description: descripcion, // ✅ Mapear a nombre correcto de columna
      tipo,
      imagen,
      curso_id,
